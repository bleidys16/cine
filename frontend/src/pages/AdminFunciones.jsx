            ...
            <div className={styles.formGrid}>
              <div className="form-group" style={{ gridColumn: '1/-1' }}>
                ...
                <select className="input" value={form.pelicula_id} onChange={e => setForm({ ...form, pelicula_id: e.target.value })} required>
                  ...
                  {peliculas.filter(p => p.estado === 'activa').map(p => (
            ...
            <tbody>
              ...
              {funciones.map(f => (
                <tr key={f.id}>
                  ...
                  <td>{f.sala}</td>
                  <td style={{ color: 'var(--accent)', fontWeight: 600 }}>${Number(f.precio).toLocaleString('es-CO')}</td>
                  <td><span className={`badge ${f.estado === 'disponible' ? 'badge-green' : 'badge-red'}`}>{f.estado}</span></td>
                  <td>
                    <button className="btn btn-ghost" style={{ padding: '6px 10px' }} onClick={() => handleEdit(f)}><Pencil size={14} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
